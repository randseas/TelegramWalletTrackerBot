...
export class MyWalletCommand {
  ...
  public async myWalletCommandHandler(msg: TelegramBot.Message) {
    ...
    const userPersonalWallet =
      await this.prismaUserRepository.getPersonalWallet(userId);
    if (!userPersonalWallet) {
      return;
    }
    const messageText =
      await this.myWalletMessages.sendMyWalletMessage(userPersonalWallet);
  ...
  public async showPrivateKeyHandler(msg: TelegramBot.Message) {
    ...
    const userPrivKey =
      await this.prismaUserRepository.showUserPrivateKey(userId);
    const messageText = `
