...
export class TokenUtils {
  ...
  public async getTokenMintAddress(tokenAddress: string) {
    try {
      ...
      console.log(
        `Error fetching mint address for token ${tokenAddress}:`,
        error
      );
      return null;
    }
  ...
  public async getTokenPriceRaydium(
    ...
    if (type === "buy") {
      ...
      const tokenAccountAddressWrappedSol = new PublicKey(
        txInstructions[0]!.info.destination
      );
      const splTokenBalance: any =
        await this.getTokenBalance(tokenAccountAddress);
      const wrappedSolBalance: any = await this.getTokenBalance(
        tokenAccountAddressWrappedSol
      );
      const solPriceInUsd: any = await this.getSolPriceNative();
      const priceOfSPLTokenInSOL =
        wrappedSolBalance / 1_000_000_000 / (splTokenBalance / 1_000_000);
