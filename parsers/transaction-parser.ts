...
export class TransactionParser {
  ...
  public async parseRpc(
    ...
    try {
      ...
      const accountKeys =
        transactionDetails[0]?.transaction.message.accountKeys;
      const signerAccount = accountKeys!.find(
      ...
      // pumpfun
      transactionDetails[0].transaction.message.instructions.map(
        (instruction: any) => {
          if (
      ...
      const nativeBalance =
        this.tokenUtils.calculateNativeBalanceChanges(transactionDetails);
      ...
      for (let i = 0; i < preBalances.length; i++) {
        ...
        if (solDifference !== 0 && i === 2 && nativeBalance?.type === "sell") {
          ...
          totalSolSwapped = Math.abs(
            (postBalances[2]! - preBalances[2]!) / 1e9
          );
        }
      }
      ...
      const raydiumTransfer =
        transactions.length > 2
          ? transactions.find(
              (t: any) => t?.info?.destination === transactions[0]?.info?.source
            )
      ...
      if (transactions.length > 1) {
        const tokenOutMint = await this.tokenUtils.getTokenMintAddress(
          transactions[0]?.info.destination
        );
        if (tokenOutMint === null) {
          return;
        }
        const tokenInMint = await this.tokenUtils.getTokenMintAddress(
          raydiumTransfer.info.source
        );
        if (tokenInMint === null) {
          return;
        }
        ...
        const cleanedTokenOutSymbol = tokenOutInfo.data.symbol.replace(
          /\x00/g,
          ""
        );
        // const tokenInInfo = await this.tokenParser.getTokenInfo(tokenInMint)
        const cleanedTokenInSymbol = tokenInInfo.data.symbol.replace(
          /\x00/g,
          ""
        );
        const formattedAmountOut = this.formatNumbers.formatTokenAmount(
          Number(transactions[0]?.info?.amount)
        );
        const formattedAmountIn = this.formatNumbers.formatTokenAmount(
          Number(raydiumTransfer?.info?.amount)
        );
        owner = parsedInfos[0]?.info?.source
          ? parsedInfos[0]?.info?.source
          : transactions[0]?.info?.authority;
        amountOut =
          cleanedTokenOutSymbol === "SOL"
            ? (Number(transactions[0]?.info?.amount) / 1e9)
                .toFixed(2)
                .toString()
            : formattedAmountOut;
        amountIn =
        ...
        if (
          transactions.length[0]?.info?.amount !== transactions[1]?.info?.amount
        ) {
          ...
          const tokenToMc =
            tokenInMint === "So11111111111111111111111111111111111111112"
              ...
              : tokenInMint;
          if (tokenPrice) {
            const tokenMarketCap = await this.tokenUtils.getTokenMktCap(
              tokenPrice,
              tokenToMc
            );
      ...
      if (
        ...
        // token out
        const tokenOutMint =
          await this.tokenUtils.getTokenMintAddressWithFallback(transactions);
        if (tokenOutMint === null) {
          return;
        }
        const tokenOutInfo = await this.tokenParser.getTokenInfo(tokenOutMint);
        ...
        const formattedAmount = this.formatNumbers.formatTokenAmount(
          Number(transactions[0]?.info?.amount)
        );
        ...
        amountOut =
          nativeBalance?.type === "sell"
            ? formattedAmount
            : totalSolSwapped.toFixed(2).toString();
        amountIn =
          nativeBalance?.type === "sell"
            ? totalSolSwapped.toFixed(2).toString()
            : formattedAmount;
        tokenOut =
          nativeBalance?.type === "sell" ? cleanedTokenOutSymbol : "SOL";
